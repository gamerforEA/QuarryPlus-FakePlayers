...
import com.gamerforea.quarryplus.FakePlayerUtils;
...
public class TilePump extends APacketTile implements IFluidHandler, IEnchantableTile
{
	...
	{
		final TileEntity te = this.worldObj.getTileEntity(this.xCoord + this.connectTo.offsetX, this.yCoord + this.connectTo.offsetY, this.zCoord + this.connectTo.offsetZ);
	...
	public void readFromNBT(final NBTTagCompound nbttc)
	{
		...
		// TODO gamerforEA code start
		String uuid = nbttc.getString("ownerUUID");
		if (!Strings.isNullOrEmpty(uuid)) this.ownerUUID = UUID.fromString(uuid);
		String name = nbttc.getString("ownerName");
	...
	public void writeToNBT(final NBTTagCompound nbttc)
	{
		...
		// TODO gamerforEA code start
		if (this.ownerUUID != null) nbttc.setString("ownerUUID", this.ownerUUID.toString());
		if (!Strings.isNullOrEmpty(this.ownerName)) nbttc.setString("ownerName", this.ownerName);
		// TODO gamerforEA code end
	}
	...
	public void updateEntity()
	{
		...
		for (final ForgeDirection fd : ForgeDirection.VALID_DIRECTIONS)
		{
			...
			if (te instanceof IFluidHandler) for (final String s : this.mapping[fd.ordinal()])
			{
	...
	public void C_recievePacket(final byte id, final byte[] data, final EntityPlayer ep)
	{
		...
		switch (id)
		{
			...
			case PacketHandler.StC_OPENGUI_MAPPING:// BI[Ljava.lang.String;
				...
				for (int i = 0; i < len; i++)
					this.mapping[target].add(badi.readUTF());
				ep.openGui(QuarryPlus.I, QuarryPlusI.guiIdPump + target, this.worldObj, this.xCoord, this.yCoord, this.zCoord);
				break;
		}
	}

	public void S_OpenGUI(final int d, final EntityPlayer ep)
	...
	public void S_changeRange(final EntityPlayer ep)
	{
		...
		else if (this.quarryRange) this.quarryRange = false;
		else this.range++;
		if (this.quarryRange) ep.addChatMessage(new ChatComponentTranslation("chat.pump_rtoggle.quarry"));
		else ep.addChatMessage(new ChatComponentTranslation("chat.pump_rtoggle.num", Integer.toString(this.range * 2 + 1)));
		this.fwt = 0;
	}
	...
	boolean S_removeLiquids(final APowerTile tbpp, final int x, final int y, final int z)
	{
			...
			do
			{
				if (this.px == -1)
				{
					...
					for (bx = 0; bx < this.block_side_x; bx++)
						for (bz = 0; bz < this.block_side_z; bz++)
							if ((this.blocks[this.py - this.yOffset][bx][bz] & 0x40) != 0)
							{
								...
								if (isLiquid(bb, false, null, 0, 0, 0, 0)) count++;
							}
				}
				else for (bz = 0; bz < this.block_side_z; bz++)
					if (this.blocks[this.py - this.yOffset][this.px][bz] != 0)
					{
						...
						if (isLiquid(bb, true, this.worldObj, this.px + this.xOffset, this.py, bz + this.zOffset, meta)) count++;
					}
				if (count > 0) break;
			}
		...
		if (count > 0 && PowerManager.useEnergyP(tbpp, this.unbreaking, count, this.px == -1 ? count : 0)) if (this.px == -1)
		{
			int bx;
			for (bx = 0; bx < this.block_side_x; bx++)
				for (bz = 0; bz < this.block_side_z; bz++)
					if ((this.blocks[this.py - this.yOffset][bx][bz] & 0x40) != 0) drainBlock(bx, bz, QuarryPlusI.blockFrame);
		}
		else for (bz = 0; bz < this.block_side_z; bz++)
			if (this.blocks[this.py - this.yOffset][this.px][bz] != 0) drainBlock(this.px, bz, Blocks.air);
		S_sendNowPacket();
		return this.py < this.cy;
	}
	...
	public IChatComponent[] C_getNames()
	{
		...
		if (this.liquids.size() > 0)
		{
			...
			for (int i = 0; i < this.liquids.size(); i++)
			{
				...
				ret[i + 1] = new ChatComponentTranslation("yog.pump.liquid", new ChatComponentTranslation(s.getFluid().getUnlocalizedName(s)), Integer.toString(s.amount));
			}
		}
		else ret[0] = new ChatComponentTranslation("chat.pumpcontainno");
		return ret;
	}
	...
	public FluidTankInfo[] getTankInfo(final ForgeDirection fd)
	{
		...
		{
			if (this.liquids.size() <= 0) for (final Integer i : FluidRegistry.getRegisteredFluidIDs().values())
				ret.add(new FluidTankInfo(new FluidStack(i.intValue(), 0), Integer.MAX_VALUE));
			else for (final FluidStack fs : this.liquids)
	 ...
	 * 
	 * */
	static final boolean isLiquid(final Block b, final boolean s, final World w, final int x, final int y, final int z, final int m)
	{
	...
	{
		final Block b = this.ebses[bx >> 4][bz >> 4][this.py >> 4].getBlockByExtId(bx & 0xF, this.py & 0xF, bz & 0xF);
		final int meta = this.ebses[bx >> 4][bz >> 4][this.py >> 4].getExtBlockMetadata(bx & 0xF, this.py & 0xF, bz & 0xF);
		if (isLiquid(b, false, null, 0, 0, 0, 0))
		{
			// TODO gamerforEA code start
			if (FakePlayerUtils.callBlockBreakEvent(bx + this.xOffset, this.py, bz + this.zOffset, this.getFakePlayer()).isCancelled()) return;
			// TODO gamerforEA code end
			FluidStack fs = null;
			if (b instanceof IFluidBlock && ((IFluidBlock) b).canDrain(this.worldObj, bx + this.xOffset, this.py, bz + this.zOffset)) fs = ((IFluidBlock) b).drain(this.worldObj, bx + this.xOffset, this.py, bz + this.zOffset, true);
			else if ((b == Blocks.water || b == Blocks.flowing_water) && meta == 0) fs = new FluidStack(FluidRegistry.WATER, FluidContainerRegistry.BUCKET_VOLUME);
			else if ((b == Blocks.lava || b == Blocks.flowing_lava) && meta == 0) fs = new FluidStack(FluidRegistry.LAVA, FluidContainerRegistry.BUCKET_VOLUME);
			if (fs != null)
			{
	...
	public Map<Integer, Byte> get()
	{
		...
		if (this.fortune > 0) ret.put(Integer.valueOf(Enchantment.fortune.effectId), Byte.valueOf(this.fortune));
		if (this.unbreaking > 0) ret.put(Integer.valueOf(Enchantment.unbreaking.effectId), Byte.valueOf(this.unbreaking));
		if (this.silktouch) ret.put(Integer.valueOf(Enchantment.silkTouch.effectId), Byte.valueOf((byte) 1));
		return ret;
	}
	...
	{
		if (id == Enchantment.fortune.effectId) this.fortune = val;
		else if (id == Enchantment.unbreaking.effectId) this.unbreaking = val;
		else if (id == Enchantment.silkTouch.effectId && val > 0) this.silktouch = true;
	}
}