...
public class TileQuarry extends TileBasic
{
		...
		switch (this.now)
		{
			...
			case MOVEHEAD:
				...
				try
				{
					...
					PacketHandler.sendPacketToAround(new YogpstopPacket(bos.toByteArray(), this, PacketHandler.StC_HEAD_POS), this.worldObj.provider.dimensionId, this.xCoord, this.yCoord, this.zCoord);
				}
	...
	private boolean S_checkTarget()
	{
		...
		final Block b = this.worldObj.getChunkProvider().loadChunk(this.targetX >> 4, this.targetZ >> 4).getBlock(this.targetX & 0xF, this.targetY, this.targetZ & 0xF);
		final float h = b == null ? -1 : b.getBlockHardness(this.worldObj, this.targetX, this.targetY, this.targetZ);
		switch (this.now)
		{
			case BREAKBLOCK:
			case MOVEHEAD:
			...
			case NOTNEEDBREAK:
				...
				if (b == null || h < 0 || b.isAir(this.worldObj, this.targetX, this.targetY, this.targetZ)) return false;
				if (this.pump == ForgeDirection.UNKNOWN && TilePump.isLiquid(b, false, null, 0, 0, 0, 0)) return false;
				if (b == QuarryPlusI.blockFrame && this.worldObj.getBlockMetadata(this.targetX, this.targetY, this.targetZ) == 0)
				{
					byte flag = 0;
			...
			case MAKEFRAME:
				...
				if (b != null && b.getMaterial().isSolid() && !(b == QuarryPlusI.blockFrame && this.worldObj.getBlockMetadata(this.targetX, this.targetY, this.targetZ) == 0))
				{
				...
				if (flag > 1)
				{
					if (b == QuarryPlusI.blockFrame && this.worldObj.getBlockMetadata(this.targetX, this.targetY, this.targetZ) == 0) return false;
					return true;
				}
				return false;
		}
	...
	private void S_setNextTarget()
	{
		if (this.now == MAKEFRAME)
		{
			...
			else if (this.addX) this.targetX++;
			else this.targetX--;
			...
			if (this.xMin == this.targetX && this.zMin == this.targetZ) if (this.digged) this.digged = false;
			else this.targetY--;
		}
		else
		{
	...
	private boolean S_makeFrame()
	{
		...
		if (!FakePlayerUtils.callBlockBreakEvent(this.targetX, this.targetY, this.targetZ, this.getFakePlayer()).isCancelled())
	...
	{
		final AxisAlignedBB axis = AxisAlignedBB.getBoundingBox(this.targetX - 4, this.targetY - 4, this.targetZ - 4, this.targetX + 6, this.targetY + 6, this.targetZ + 6);
	...
	private void S_createBox()
	{
		...
		if (!S_checkIAreaProvider(this.xCoord - 1, this.yCoord, this.zCoord)) if (!S_checkIAreaProvider(this.xCoord + 1, this.yCoord, this.zCoord)) if (!S_checkIAreaProvider(this.xCoord, this.yCoord, this.zCoord - 1)) if (!S_checkIAreaProvider(this.xCoord, this.yCoord, this.zCoord + 1)) if (!S_checkIAreaProvider(this.xCoord, this.yCoord - 1, this.zCoord)) if (!S_checkIAreaProvider(this.xCoord, this.yCoord + 1, this.zCoord))
		{
			final ForgeDirection o = ForgeDirection.values()[this.worldObj.getBlockMetadata(this.xCoord, this.yCoord, this.zCoord)].getOpposite();
			switch (o)
			{
				case EAST:
	...
	private boolean S_checkIAreaProvider(final int x, final int y, final int z)
	{
		...
		if (ModAPIManager.INSTANCE.hasAPI("BuildCraftAPI|core") && te instanceof IAreaProvider)
		{
			...
			if (this.xCoord >= this.xMin && this.xCoord <= this.xMax && this.yCoord >= this.yMin && this.yCoord <= this.yMax && this.zCoord >= this.zMin && this.zCoord <= this.zMax)
			{
	...
	{
		if (this.worldObj.getBlock(x, y, z) == QuarryPlusI.blockFrame) this.worldObj.setBlockMetadataWithNotify(x, y, z, 1, 3);
	}
	...
	public void G_renew_powerConfigure()
	{
		...
		{
			final TileEntity te = this.worldObj.getTileEntity(this.xCoord + this.pump.offsetX, this.yCoord + this.pump.offsetY, this.zCoord + this.pump.offsetZ);
	...
	{
		final AxisAlignedBB aabb = AxisAlignedBB.getBoundingBox(this.xCoord, Double.NEGATIVE_INFINITY, this.zCoord, this.xCoord + 1, this.yCoord + 1, this.zCoord + 1);
		if (this.yMax == Integer.MIN_VALUE) return aabb;
		...
		if (xn < aabb.minX) aabb.minX = xn;
		if (xx > aabb.maxX) aabb.maxX = xx;
		if (aabb.maxY < yx) aabb.maxY = yx;
		if (zn < aabb.minZ) aabb.minZ = zn;
		if (zx > aabb.maxZ) aabb.maxZ = zx;
		return aabb;
	}
}