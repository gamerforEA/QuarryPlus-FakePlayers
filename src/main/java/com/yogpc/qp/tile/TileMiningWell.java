...
public class TileMiningWell extends TileBasic
{
	...
	private boolean S_checkTarget(final int depth)
	{
		...
		final Block b = this.worldObj.getChunkProvider().loadChunk(this.xCoord >> 4, this.zCoord >> 4).getBlock(this.xCoord & 0xF, depth, this.zCoord & 0xF);
		final float h = b == null ? -1 : b.getBlockHardness(this.worldObj, this.xCoord, depth, this.zCoord);
		if (b == null || h < 0 || b == QuarryPlusI.blockPlainPipe || b.isAir(this.worldObj, this.xCoord, depth, this.zCoord)) return false;
	...
	protected void G_destroy()
	{
		...
		{
			if (this.worldObj.getBlock(this.xCoord, depth, this.zCoord) != QuarryPlusI.blockPlainPipe) break;
			// TODO gamerforEA code start
			if (FakePlayerUtils.callBlockBreakEvent(this.xCoord, depth, this.zCoord, this.getFakePlayer()).isCancelled()) continue;
			// TODO gamerforEA code end
