...
public abstract class TileBasic extends APowerTile implements IEnchantableTile, IInventory, IInventoryConnection
{
	...
	protected boolean S_breakBlock(final int x, final int y, final int z)
	{
		...
		if (!PowerManager.useEnergyB(this, b.getBlockHardness(this.worldObj, x, y, z), S_addDroppedItems(dropped, b, x, y, z), this.unbreaking)) return false;
	...
	private byte S_addDroppedItems(final Collection<ItemStack> list, final Block b, final int x, final int y, final int z)
	{
		...
		if (b.canSilkHarvest(this.worldObj, null, x, y, z, meta) && this.silktouch && this.silktouchList.contains(new BlockData(GameData.getBlockRegistry().getNameForObject(b), meta)) == this.silktouchInclude)
		{
	...
	public static final Method createStackedBlock = ReflectionHelper.getMethod(Block.class, new String[] { "func_149644_j", "createStackedBlock" }, new Class<?>[] { int.class });
	...
	public Map<Integer, Byte> get()
	{
		...
		if (this.efficiency > 0) ret.put(Integer.valueOf(Enchantment.efficiency.effectId), Byte.valueOf(this.efficiency));
	...
	{
		if (id == Enchantment.efficiency.effectId) this.efficiency = val;
		else if (id == Enchantment.fortune.effectId) this.fortune = val;
	...
	{// NOTE better way?
		return i < 0 || i >= this.cacheItems.size() ? new ItemStack(Blocks.cobblestone, 0) : this.cacheItems.get(i);
	}

	@Override
	public ItemStack decrStackSize(final int i, final int a)
	{
		if (i < 0 || i >= this.cacheItems.size()) return null;
	...
	{
		if (is != null && is.stackSize > 0) System.err.println("QuarryPlus WARN: call setInventorySlotContents with non null ItemStack.");
		if (i >= 0 && i < this.cacheItems.size()) this.cacheItems.remove(i);
	}
