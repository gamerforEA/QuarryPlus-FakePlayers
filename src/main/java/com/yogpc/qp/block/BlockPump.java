...
public class BlockPump extends ADismCBlock
{
	...
	public void onBlockPlacedBy(final World w, final int x, final int y, final int z, final EntityLivingBase el, final ItemStack is)
	{
		...
		if (el instanceof EntityPlayer)
		{
			...
			TilePump tile = (TilePump) w.getTileEntity(x, y, z);
	...
	public boolean onBlockActivated(final World world, final int x, final int y, final int z, final EntityPlayer ep, final int side, final float par7, final float par8, final float par9)
	{
		...
		if (equipped instanceof ItemTool)
		{
			...
			{
				if (world.isRemote) return true;
				for (final IChatComponent s : EnchantmentHelper.getEnchantmentsChat((IEnchantableTile) world.getTileEntity(x, y, z)))
					ep.addChatMessage(s);
